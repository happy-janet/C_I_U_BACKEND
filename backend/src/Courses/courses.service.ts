...
export class CoursesService {
  ...
  async create(createCourseDto: CreateCourseDto) {
    ...
    if (existingCourse) {
      throw new ConflictException(
        'Course with this name already exists in the specified faculty',
      );
    }
  ...
  async updateCourse(id: string, updateCourseDto: UpdateCourseDto) {
    ...
    const user = await this.prisma.courses.findUnique({
      where: { id: Number(id) },
    });
    if (!user) {
  ...
  async findAll() {
    ...
    return courses.map((course) => ({
      ...course,
      listFieldName: Array.isArray(course.courseUnitCode)
        ? course.courseUnitCode
        : [course.courseUnitCode],
    }));
  }
  ...
  async getCourseUnitCount() {
    ...
    const allCourseUnits = uniqueCourseUnits.flatMap(
      (course) => course.courseUnits,
    );
